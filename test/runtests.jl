...
@testset "Optimising by Charibde (A hybrid approach) using only one worker" begin
      ...
      (global_min, minimisers, info) = charibde_min((x,y)->x^2+y+1, IntervalBox(1..2, 2..3), workers = 1)
      @test global_min ⊆ 4.0 .. 4.01
      @test minimisers[1] ⊆ (1..1.001) × (2..2.001)

      (global_min, minimisers, info)= charibde_min((x,y)->x^2 + y^2, IntervalBox(2..3, 3..4), workers = 1)
      @test global_min ⊆ 13 .. 13.01
...
@testset "Optimising by Charibde (A hybrid approach) using 2 workers" begin
      (global_min, minimisers, info) = charibde_min((x,y)->x^3 + 2y + 5, IntervalBox(2..4, 2..3))
