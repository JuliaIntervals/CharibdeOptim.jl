...
This package gives you the global optimum value (max or min) of any mathematical objective function in a given domain by running two different algorithms (*Interval Branch & Contract* and *Differential evolution* which is a generic algorithm) in parallel by keep exchanging there best known solution through `channels` or `remotechannels`, this approach of solving mathematical optimum value problems of both constrained and unconstrained type is called *Charibde* which can be used through functions `charibde_min` or `charibde_max` (to get minimum or maximum value of the objective function).

The package also provides the function `ibc_minimise`/`ibc_maximise` to use just *Interval Branch & Contract* algorithm to solve problems.

To run Charibde on two workers(processors) there are few requirements

1. The Julia session should have at least 2 more workers other than the master processor with package `CharibdeOptim` loaded on them,  which can we have through `Distributed` package, like this  
  ```julia
...
While using the package through JuMP, you really not have to care about all these, you just have to tell on how many workers (through `workers` keyword argument which is by default set to 2) you want to run *Charibde* and the package automatically add required number of workers and load and define all the required packages and stuff on them.  

`charibde_min` supports following keyword arguments,

1. `structure`
   - `ibc_minimise`/`ibc_maximise` also supports it.
...
5. `debug`
   - You have to set it to `true` to access debug mode of `ibc_minimise`/`ibc_maximise` or `charibde_min`/`charibde_max`.

## Acknowledge constraints

In case of Constrained Optimisation, you need to define constraints and make them acknowledged in *Charibde* or *IBC*.
To define each constraint the package provides the function `constraint` which takes four arguments
  ...
  - keyword argument `epsilon` which define tolerance in case of equality constraint (default is set to *1e-4*)
...
After defining every constraint using function `constraint` you need to pass all returned objects (of type `Constraint`) in a Vector along with objective function and search-space in `charibde_max`/`charibde_min` or `ibc_maximise`/`ibc_minimise`  according to the required optimum value (minimum or maximum).
